        ...
        <symbol id="exclamation-triangle-fill" fill="currentColor" viewBox="0 0 16 16">
          <path d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z" />
			...
			<div class="container-fluid">
                @if(!empty(Auth::user()->role) && Auth::user()->role != 1 && !empty($user_kyc->admin_kyc_onboarding) && $user_kyc->admin_kyc_onboarding == 1)
                	<div class="row page-titles">
					<ol class="breadcrumb">
						{{-- <li class="breadcrumb-item"><a href="javascript:void(0)">Form</a></li>
						<li class="breadcrumb-item active"><a href="javascript:void(0)">Wizard</a></li> --}}

                       @if(!empty($user_kyc) && isset($user_kyc->approval_status) && $user_kyc->approval_status == 0)

                            <div class="alert alert-primary kyc-alert animated-text" role="alert">
                              <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Info:">
                                <use xlink:href="#info-fill" />
                              </svg>
                              <strong>KYC Required:</strong> To access certain features, please complete your KYC process.
                              <a href="{{ route('auth.user_kyc') }}" class="btn btn-primary btn-sm float-end">Complete KYC</a>
                            </div>

                          @elseif (!empty($user_kyc) && isset($user_kyc->approval_status) && $user_kyc->approval_status == 1)
                            <div class="alert alert-warning d-flex align-items-center animated-text" role="alert">
                              <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Warning:">
                                <use xlink:href="#exclamation-triangle-fill" />
                              </svg>
                              KYC Pending: Your KYC application is pending approval by the support team.
                            </div>
                          @elseif (!empty($user_kyc) && isset($user_kyc->approval_status) && $user_kyc->approval_status == 3)
                            <div class="alert alert-danger d-flex align-items-center animated-text" role="alert">
                              <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:">
                                <use xlink:href="#exclamation-triangle-fill" />
                              </svg>
                              KYC Rejected: Your KYC application has been rejected. Please review and resubmit your documents.
                            </div>
                          @endif
